...
class Order:
    ...
    Order 类：
      ...
      - theoretical_delivery_time: 理论送达时间 = distance / Default LOGISTICS_SPEED
    ...
    def __init__(self, order_id, creation_time, sender_id, receiver_id,
                quantity, order_type, distance, logistics_speed):
        ...
        # 理论送达时间，单位：时间步
        self.theoretical_delivery_time = math.ceil(distance / 1000)
...
class Manufacturer(Agent):
    ...
    def __init__(self, unique_id, model, pos, status,
        ...
        # ---- 50 步窗口增量 ----
        self.hist_product  = deque(maxlen=100)   # 每步新增成品仓储费
        self.hist_material = deque(maxlen=100)   # 每步新增原料仓储费
        self.hist_trans    = deque(maxlen=100)   # 每步新增运输费
        self.hist_penalty  = deque(maxlen=100)   # 每步新增违约费
        self.hist_finish   = deque(maxlen=100)   # 每步新增完成量
    ...
    def order_raw_material(self, raw_material_needed):
        ...
        all_ss = [a for a in self.model.schedule.agents if isinstance(a, Supplier)]
        if not all_ss:
            return
        ys = [s for s in all_ss if s.status == 'Y']

        # 用已有的 distance_to_supplier
        def pick_nearest(cands, k=1):
            lst = []
            for s in cands:
                d = self.distance_to_supplier[s.unique_id]
                lst.append((d, s))
            random.shuffle(lst)
            lst.sort(key=lambda x: x[0])
            return [s for _, s in lst[:k]]
        ...
        if self.material_order_mode == "normal": #or len(valid_suppliers) < 2:
            ...
            chosen.receive_order(raw_order)
            self.model.pending_deliveries.append(raw_order)
        elif self.material_order_mode == "multi_s":
            # 先挑状态为Y的
            if len(ys) >= 2:
                suppliers = random.sample(ys, 2)
            elif len(ys) == 1:
                first = ys[0]
                rest = [s for s in all_ss if s is not first]
                second = pick_nearest(rest, 1)[0]
                suppliers = [first, second]
            else:
                suppliers = pick_nearest(all_ss, 2)
    ...
    def step(self):
        ...
        #self.cumulative_storage_cost_material += self.raw_material_inventory*0.4
         #—— 当步仓储费（增量）——
        p_cost = self.product_inventory * 1
        r_cost = self.raw_material_inventory * 0.4
    
        self.hist_product.append(p_cost)
        self.hist_material.append(r_cost)
    
        # 同时仍要累计
        self.cumulative_storage_cost_product  += p_cost
        self.cumulative_storage_cost_material += r_cost
...
class Supplier(Agent):
    ...
    def production_logic(self):
        ...
        if gap < 0:
            self.status = 'N'
            production_needed = min(abs(gap)+50, self.material_capacity)
